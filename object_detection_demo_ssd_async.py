    ...
    while cap.isOpened():
        ...
        if exec_net.requests[cur_request_id].wait(-1) == 0:
            ...
            for coord in coordlist:
                ...
                frame = cv2.circle(frame, (coord[0], coord[1]), new_depth , color_blue, thickness=2)
                image_tracking = cv2.circle(image_tracking, (coord[0], round(coord[2]*2)), 3, color_blue, thickness=1)

            # adding text
            ppl_message = "People detected: {}".format(n_ppl)
            cv2.putText(frame, ppl_message, (30, round(frame_h - 50)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
            ...
            if (n_ppl > 7 and frame_waiter<0 ):
                frame_waiter = 100
                cv2.putText(frame, ppl_message_alert, (100, round(frame_h / 2)), cv2.FONT_HERSHEY_SIMPLEX, 1.5,
                            (0, 0, 255), 3)
